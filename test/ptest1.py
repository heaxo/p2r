...
def process_one_image(args) -> Dict[str, Any]:
    ...
    mask_paths: Dict[str, str] = {"canonical_image": str(canonical_image_path)}

    # 1. paper：YOLO -> 一点 -> SAM2
    ...
    mask_paths["plate_final_with_paper_fill"] = save_mask(final_plate_mask, run_dir / "plate_final_with_paper_fill.png")
    ...
    # 5. 钢板外轮廓 -> mm 坐标 -> 尺寸/DXF
    plate_outer_contour = _largest_contour_from_mask(final_plate_mask)
    if plate_outer_contour is None or len(plate_outer_contour) < 4:
        raise RuntimeError("未能从最终 plate mask 中提取钢板外轮廓")
    plate_contour_px = plate_outer_contour.reshape(-1, 2).astype(np.float32)

    H, paper_quad_mm, a4_size, used_orientation = build_a4_homography(paper_quad_px, orientation=args.a4_orientation)
    ...
    write_simple_dxf(dxf_path, plate_contour_mm, offset_to_positive=True)

    debug_overlay_path = run_dir / "debug_overlay.jpg"
    save_debug_overlay(debug_overlay_path, image_rgb, plate_contour_px, paper_quad_px, plate_point_info, paper_info, dims)
    mask_paths["debug_overlay"] = str(debug_overlay_path)

    mm_preview_path = run_dir / "debug_mm_preview.png"
    save_mm_preview(mm_preview_path, plate_contour_mm, paper_quad_mm, dims)
    ...
    output = {
        ...
        "note": "尺寸基于单张 A4 的平面 Homography 换算。DXF只包含钢板外轮廓，不包含A4纸轮廓。A4 与钢板不共面、钢板弯曲、A4角点不准、相机广角畸变都会引入误差。若尺寸偏差明显，优先传 --paper-points 使用准确A4四角，或指定 --a4-orientation landscape/portrait。",
    }
    result_json_path.write_text(json.dumps(json_safe(output), ensure_ascii=False, indent=2), encoding="utf-8")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--image", required=False, help="输入图片路径", default=r"D:\lantek\project-python\p2r\test\batch_output\val\B2212013-001.jpg")
