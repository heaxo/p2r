# -*- coding: utf-8 -*-
"""
YOLO + SAM2 钢板/A4纸识别 HTTP 服务版 v4
...
启动：
    ...
    python plate_yolo_sam2_http_service_v4.py \
...
class ProcessRequest(BaseModel):
    ...
    # 透视校正。auto 表示根据 A4纸四边形自动判断是否需要校正。
...
def find_paper_quad_from_mask(paper_mask: np.ndarray) -> Dict[str, Any]:
    """
    从 paper mask 中提取 A4 纸四角。

    优先使用轮廓多边形近似得到真实四边形；失败时退回最小外接旋转矩形。
    真实斜拍场景下，多边形四角比 minAreaRect 更适合做透视校正。
    """
    contour = _largest_contour_from_mask(paper_mask)
    if contour is None or cv2.contourArea(contour) <= 1:
    ...
    if perimeter <= 1:
        raise RuntimeError("无法做透视校正：paper 轮廓周长异常")

    # 在多个 epsilon 中寻找四边形。epsilon 太小会保留锯齿，太大会丢角。
    best_quad = None
    best_score = None
    best_epsilon = None
    for factor in [0.008, 0.010, 0.012, 0.015, 0.018, 0.020, 0.025, 0.030, 0.040, 0.050, 0.060, 0.080]:
        ...
        if best_quad is None or score < best_score:
            best_quad = pts
            best_score = score
            best_epsilon = factor
...
def build_perspective_correction_plan(
    ...
    quad_info = find_paper_quad_from_mask(paper_mask)
    quad = quad_info["quad"]
    metrics = get_quad_metrics(quad, float(req.a4_width_mm), float(req.a4_height_mm))

    trapezoid_delta = float(metrics["trapezoid_delta"])
    aspect_error = float(metrics["aspect_error"])
    need_by_auto = (
...
def build_a4_scale_from_perspective_plan(plan: Dict[str, Any], req: ProcessRequest) -> Dict[str, Any]:
    ...
    short_mm = float(min(req.a4_width_mm, req.a4_height_mm))
    long_mm = float(max(req.a4_width_mm, req.a4_height_mm))
    ...
    if req.enable_dxf and plate_for_dxf is not None:
        try:
            ...
            if perspective_plan.get("enabled"):
                # 斜拍时使用 A4 四角做透视校正，然后在校正后的 mask 上计算尺寸和 DXF。
...
# FastAPI
# =========================
app = FastAPI(title="YOLO + SAM2 Plate/Paper Service", version="1.2.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
