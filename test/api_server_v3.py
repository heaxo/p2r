# -*- coding: utf-8 -*-
"""
YOLO + SAM2 钢板/A4纸识别 HTTP 服务版 v6
...
4. 尺寸和 DXF：
   ...
   - 生成毫米单位 DXF，并返回 dxf_path、mask_paths、dimensions，不返回 DXF 内容
5. v5 新增稳健 A4 拟合：
   - paper mask 先做闭运算、开运算、填洞和最大连通域保留
   - 轻微变形的 A4 纸不再直接相信原始毛边
   - 可强制按 210:297 比例拟合成理想 A4 四边形，再做透视校正和尺寸计算
6. v6 新增标定板优先方案：
   - 支持 OpenCV ArUco 四角标定板
   - 识别时优先使用 ArUco 角点做透视校正和毫米坐标标定
   - ArUco 失败时可自动回退到普通 A4 paper mask 方案
   - 支持可选相机去畸变，先 undistort 再做 YOLO/SAM2/标定
...
class ProcessRequest(BaseModel):
    ...
    # 第4版开始接口不再返回 DXF 文本内容。该字段保留仅用于兼容旧请求，不参与响应。
    dxf_include_content: bool = False

    # 相机去畸变。用于处理手机广角、近距离拍摄导致的桶形/枕形畸变。
    # calibration 文件支持 json 或 npz：
    # json: {"camera_matrix": [[...]], "dist_coeffs": [...]}
    # npz: 包含 camera_matrix 和 dist_coeffs 两个数组。
    enable_camera_undistort: bool = False
    camera_calibration_path: Optional[str] = None
    camera_undistort_alpha: float = 0.0

    # 透视校正。auto 表示根据 A4纸四边形或 ArUco 标定板自动判断是否需要校正。
    enable_perspective_correction: bool = True
    perspective_mode: str = "auto"  # auto / always / never
    perspective_px_per_mm: Optional[float] = None
    perspective_margin_px: int = 80
    perspective_max_output_pixels: int = 40000000
    perspective_auto_trapezoid_threshold: float = 0.06
    perspective_auto_aspect_threshold: float = 0.08

    # v6：标定方式。
    # auto：优先 ArUco，失败后回退普通 A4 paper mask。
    # aruco：只用 ArUco 标定板，失败就报错。
    # paper：只用普通 A4 paper mask。
    calibration_mode: str = "auto"

    # v6：OpenCV ArUco 标定板。
    # 默认假设打印一张 A4 纸，四角分别放 marker id 0/1/2/3。
    # id 顺序：左上、右上、右下、左下。
    enable_aruco_calibration: bool = True
    aruco_dictionary_name: str = "DICT_4X4_50"
    aruco_marker_ids: List[int] = Field(default_factory=lambda: [0, 1, 2, 3])
    aruco_board_width_mm: float = 210.0
    aruco_board_height_mm: float = 297.0
    aruco_marker_size_mm: float = 35.0
    aruco_marker_margin_mm: float = 12.0
    aruco_min_markers: int = 2
    save_aruco_debug_images: bool = True

    # v5：A4纸稳健拟合。用于处理 paper mask 毛边、轻微翘角、轻微变形导致的四角不准。
    # robust_fit：清理 paper mask 后优先拟合标准 A4 比例矩形；
    # approx_poly：尽量使用轮廓四边形；
    # min_area_rect：直接使用最小外接旋转矩形；
    # raw：兼容 v4 逻辑，直接从原始 paper mask 取四边形。
    paper_rect_mode: str = "robust_fit"
    force_a4_aspect_ratio: bool = True
    paper_rect_clean_kernel: int = 15
    paper_rect_open_kernel: int = 5
    paper_rect_quality_threshold: float = 0.55
    fallback_when_paper_deformed: str = "robust_fit"  # robust_fit / no_perspective / raw
    save_paper_rect_masks: bool = True
...
class ArucoBoardRequest(BaseModel):
    """生成 A4 ArUco 标定板的请求参数。"""
    output_root: Optional[str] = Field(default=None, description="输出目录；不传则使用启动参数")
    ...
    marker_margin_mm: float = 12.0
    dpi: int = 300
    filename: str = "aruco_a4_board.png"
...
def _get_aruco_module():
    """获取 OpenCV ArUco 模块。需要安装 opencv-contrib-python。"""
    aruco = getattr(cv2, "aruco", None)
    if aruco is None:
        raise RuntimeError("当前 OpenCV 没有 aruco 模块，请安装 opencv-contrib-python，而不是只安装 opencv-python")
    return aruco
...
def get_aruco_dictionary(dictionary_name: str):
    ...
    name = (dictionary_name or "DICT_4X4_50").strip()
    if not hasattr(aruco, name):
        supported = [k for k in dir(aruco) if k.startswith("DICT_")]
        raise RuntimeError(f"不支持的 ArUco 字典：{name}，当前 OpenCV 支持：{supported[:30]}")
    return aruco.getPredefinedDictionary(getattr(aruco, name))


def generate_aruco_marker_image(dictionary_name: str, marker_id: int, side_pixels: int) -> np.ndarray:
    """生成单个 ArUco marker 图像，兼容不同 OpenCV 版本。"""
    aruco = _get_aruco_module()
    dictionary = get_aruco_dictionary(dictionary_name)
    side_pixels = max(20, int(side_pixels))
    if hasattr(aruco, "generateImageMarker"):
        marker = aruco.generateImageMarker(dictionary, int(marker_id), side_pixels)
    else:
        marker = np.zeros((side_pixels, side_pixels), dtype=np.uint8)
        aruco.drawMarker(dictionary, int(marker_id), side_pixels, marker, 1)
    return marker.astype(np.uint8)


def get_aruco_marker_object_corners_mm(
    marker_id: int,
    ...
    """
    返回指定 marker 在 A4 标定板真实坐标系中的四个角。

    marker_ids 顺序固定为：左上、右上、右下、左下。
    坐标单位为 mm，原点是 A4 左上角，x 向右，y 向下。
    """
    ids = [int(x) for x in marker_ids]
    if len(ids) < 4:
        raise RuntimeError("aruco_marker_ids 至少需要 4 个，顺序为左上、右上、右下、左下")

    marker_id = int(marker_id)
    if marker_id not in ids[:4]:
        return None

    w = float(board_width_mm)
    h = float(board_height_mm)
    s = float(marker_size_mm)
    m = float(marker_margin_mm)
    if s <= 1 or m < 0 or w <= s * 2 or h <= s * 2:
        raise RuntimeError("ArUco 标定板参数不合理，请检查 board_width/height、marker_size、marker_margin")
...
def detect_aruco_markers(
    ...
    detected = []
    if ids is not None:
        for idx, corner in zip(ids.reshape(-1), corners):
            detected.append({
                "id": int(idx),
                "corners_px": np.asarray(corner, dtype=np.float32).reshape(4, 2),
            })

    return {
        "detected": detected,
        "detected_ids": [int(x["id"]) for x in detected],
        "rejected_count": int(len(rejected)) if rejected is not None else 0,
    }


def save_aruco_debug_image(
    image_rgb: np.ndarray,
    detected_markers: Sequence[Dict[str, Any]],
    output_path: str | Path,
) -> str:
    """保存 ArUco 检测调试图。"""
    img = image_rgb.copy().astype(np.uint8)
    bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    aruco = getattr(cv2, "aruco", None)
    if aruco is not None and detected_markers:
        corners = [np.asarray(m["corners_px"], dtype=np.float32).reshape(1, 4, 2) for m in detected_markers]
        ids = np.asarray([[int(m["id"])] for m in detected_markers], dtype=np.int32)
        aruco.drawDetectedMarkers(bgr, corners, ids)
    else:
        for m in detected_markers:
            pts = np.asarray(m["corners_px"], dtype=np.int32).reshape(4, 2)
            cv2.polylines(bgr, [pts], True, (0, 255, 0), 3)
            cv2.putText(bgr, str(m["id"]), tuple(pts[0]), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
    out = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
    return save_image_rgb(out, output_path)


def generate_a4_aruco_board_image(
    dictionary_name: str = "DICT_4X4_50",
    ...
    """生成 A4 四角 ArUco 标定板。打印时必须按 100% 原尺寸打印。"""
    ids = [int(x) for x in marker_ids]
    if len(ids) < 4:
        raise RuntimeError("marker_ids 至少需要 4 个，顺序为左上、右上、右下、左下")

    px_per_mm = float(dpi) / 25.4
    w_px = int(round(float(board_width_mm) * px_per_mm))
    h_px = int(round(float(board_height_mm) * px_per_mm))
    s_px = int(round(float(marker_size_mm) * px_per_mm))
    m_px = int(round(float(marker_margin_mm) * px_per_mm))

    board = np.full((h_px, w_px), 255, dtype=np.uint8)
    positions = [
        (m_px, m_px),
        (w_px - m_px - s_px, m_px),
        (w_px - m_px - s_px, h_px - m_px - s_px),
        (m_px, h_px - m_px - s_px),
    ]
    for marker_id, (x, y) in zip(ids[:4], positions):
        marker = generate_aruco_marker_image(dictionary_name, marker_id, s_px)
        board[y:y + s_px, x:x + s_px] = marker

    rgb = cv2.cvtColor(board, cv2.COLOR_GRAY2RGB)
    pil = Image.fromarray(rgb)
    draw = ImageDraw.Draw(pil)
    note = "Print at 100% scale. Marker order: 0 TL, 1 TR, 2 BR, 3 BL."
    draw.text((m_px, h_px - max(20, m_px // 2)), note, fill=(0, 0, 0))
    return np.asarray(pil).copy()


def load_camera_calibration(calibration_path: str | Path) -> Tuple[np.ndarray, np.ndarray]:
    """读取相机标定参数。支持 json 和 npz。"""
    path = Path(calibration_path)
    if not path.exists():
        raise RuntimeError(f"相机标定文件不存在：{path}")
    ...
    if path.suffix.lower() == ".npz":
        ...
        dist_coeffs = np.asarray(payload.get("dist_coeffs"), dtype=np.float64)

    if camera_matrix.shape != (3, 3):
        raise RuntimeError("camera_matrix 必须是 3x3 矩阵")
    if dist_coeffs.size < 4:
        raise RuntimeError("dist_coeffs 至少需要 4 个参数")
    return camera_matrix, dist_coeffs.reshape(-1, 1)


def undistort_image_rgb(
    image_rgb: np.ndarray,
    calibration_path: str | Path,
    alpha: float = 0.0,
) -> Tuple[np.ndarray, Dict[str, Any]]:
    """用相机标定参数去畸变。alpha=0 会尽量裁掉黑边，alpha=1 保留更多视野。"""
    ...
    undistorted_bgr = cv2.undistort(bgr, camera_matrix, dist_coeffs, None, new_camera_matrix)
    x, y, rw, rh = [int(v) for v in roi]
    if rw > 10 and rh > 10 and alpha <= 0.01:
        undistorted_bgr = undistorted_bgr[y:y + rh, x:x + rw]
    undistorted_rgb = cv2.cvtColor(undistorted_bgr, cv2.COLOR_BGR2RGB)
    return undistorted_rgb.copy(), {
        "enabled": True,
        "calibration_path": str(calibration_path),
        "alpha": alpha,
        "original_shape": [int(h), int(w)],
        "output_shape": [int(undistorted_rgb.shape[0]), int(undistorted_rgb.shape[1])],
...
def _keep_largest_component(bin_mask: np.ndarray) -> np.ndarray:
    """只保留最大连通区域，避免 paper mask 的小噪点影响 A4 矩形拟合。"""
    src = (_binary_mask(bin_mask) > 0).astype(np.uint8)
    if src.size == 0 or int(src.sum()) <= 0:
        return src
...
def clean_paper_mask_for_rect(
    ...
    目的：SAM2/YOLO 得到的 paper mask 可能存在毛边、缺口、文字干扰、轻微翘角阴影。
    这里先做闭运算补边，再做开运算去小毛刺，然后填洞并只保留最大连通区域。
    """
    out = (_binary_mask(paper_mask) > 0).astype(np.uint8)
    if out.size == 0 or int(out.sum()) <= 0:
        return out * 255

    close_k = _odd_kernel_size(close_kernel_size, min_value=3)
    if close_k > 0:
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (close_k, close_k))
        out = cv2.morphologyEx(out, cv2.MORPH_CLOSE, kernel, iterations=1)

    open_k = _odd_kernel_size(open_kernel_size, min_value=3)
    if open_k > 0:
...
def make_quad_mask(image_shape: Tuple[int, int], quad: np.ndarray) -> np.ndarray:
    """根据四边形生成 mask，用于保存稳健拟合后的 A4 矩形。"""
    h, w = image_shape[:2]
    out = np.zeros((h, w), dtype=np.uint8)
    pts = np.asarray(quad, dtype=np.float32).reshape(4, 2)
    cv2.fillConvexPoly(out, np.round(pts).astype(np.int32), 255)
    return out


def _try_find_approx_quad_from_contour(contour: np.ndarray, contour_area: float) -> Optional[Dict[str, Any]]:
    """从轮廓中尝试寻找四边形近似。"""
    ...
    best_score = None
    best_epsilon = None
    for factor in [0.006, 0.008, 0.010, 0.012, 0.015, 0.018, 0.020, 0.025, 0.030, 0.040, 0.050, 0.060, 0.080]:
        ...
        if area <= 1:
            continue
        score = abs(area - contour_area) / max(1.0, contour_area)
        if best_quad is None or score < float(best_score):
            best_quad = pts
            best_score = score
            best_epsilon = factor
    ...
    return {
        "quad": order_quad_points(best_quad),
        "method": "approx_poly_cleaned",
        "epsilon_factor": float(best_epsilon),
        "area_error_ratio": float(best_score),
    }


def _unit_vector(vec: np.ndarray, fallback: np.ndarray) -> np.ndarray:
    """返回单位向量，长度异常时使用 fallback。"""
...
def build_forced_a4_quad_from_contour(
    contour: np.ndarray,
    a4_width_mm: float,
    a4_height_mm: float,
) -> Dict[str, Any]:
    """
    根据 paper 轮廓拟合一个标准 A4 比例的理想矩形。

    这个函数不直接相信 paper mask 的毛边，而是使用最小外接旋转矩形确定中心、方向和大致尺寸，
    再按 A4 真实长宽比重新构造四个角。
    """
    rect = cv2.minAreaRect(contour)
    raw_box = order_quad_points(cv2.boxPoints(rect).astype(np.float32))

    top = _distance(raw_box[0], raw_box[1])
    bottom = _distance(raw_box[2], raw_box[3])
    left = _distance(raw_box[3], raw_box[0])
    right = _distance(raw_box[1], raw_box[2])
    h_len = max(1.0, (top + bottom) / 2.0)
    v_len = max(1.0, (left + right) / 2.0)
    ...
    px_per_mm_h = h_len / max(1e-6, h_mm)
    px_per_mm_v = v_len / max(1e-6, v_mm)
    px_per_mm = float((px_per_mm_h + px_per_mm_v) / 2.0)

    ideal_h_len = float(h_mm * px_per_mm)
    ideal_v_len = float(v_mm * px_per_mm)

    horizontal_dir = _unit_vector(
        (raw_box[1] - raw_box[0]) + (raw_box[2] - raw_box[3]),
        raw_box[1] - raw_box[0],
    )
    vertical_dir = _unit_vector(
        (raw_box[3] - raw_box[0]) + (raw_box[2] - raw_box[1]),
        raw_box[3] - raw_box[0],
    )

    center = raw_box.mean(axis=0)
    half_h = horizontal_dir * (ideal_h_len / 2.0)
    half_v = vertical_dir * (ideal_v_len / 2.0)
    ...
    forced_area = float(abs(cv2.contourArea(quad)))
    fill_ratio = float(contour_area / max(1.0, forced_area))

    return {
        "quad": quad,
        "method": "forced_a4_aspect_min_area_rect",
        "raw_min_area_rect_points_px": [[float(x), float(y)] for x, y in raw_box],
        "px_per_mm_from_rect": px_per_mm,
        "raw_horizontal_px": float(h_len),
        "raw_vertical_px": float(v_len),
        "ideal_horizontal_px": float(ideal_h_len),
        "ideal_vertical_px": float(ideal_v_len),
        "contour_to_forced_rect_area_ratio": fill_ratio,
    }


def _legacy_find_paper_quad_from_contour(contour: np.ndarray) -> Dict[str, Any]:
    """兼容 v4 的四边形提取逻辑：优先 approxPolyDP，失败退回 minAreaRect。"""
    contour_area = float(abs(cv2.contourArea(contour)))
    approx = _try_find_approx_quad_from_contour(contour, contour_area)
...
def calculate_a4_scale_from_paper_mask(
    ...
    v5 默认不直接使用 paper mask 的毛边，而是先拟合一个稳健 A4 四边形。
    这样 A4 纸轻微翘边、边缘分割不平滑、角点有毛刺时，mm_per_px 更稳定。
    """
    quad_info = find_paper_quad_from_mask(
        paper_mask=paper_mask,
        a4_width_mm=float(a4_width_mm),
        a4_height_mm=float(a4_height_mm),
        paper_rect_mode=paper_rect_mode,
        force_a4_aspect_ratio=force_a4_aspect_ratio,
        close_kernel_size=int(paper_rect_clean_kernel),
        open_kernel_size=int(paper_rect_open_kernel),
        quality_threshold=0.0,
        fallback_when_deformed="robust_fit",
    )
    quad = np.asarray(quad_info["quad"], dtype=np.float32).reshape(4, 2)

    top = _distance(quad[0], quad[1])
    right = _distance(quad[1], quad[2])
    bottom = _distance(quad[2], quad[3])
    left = _distance(quad[3], quad[0])

    horizontal_avg = max(1e-6, (top + bottom) / 2.0)
    vertical_avg = max(1e-6, (left + right) / 2.0)

    short_px = min(horizontal_avg, vertical_avg)
    long_px = max(horizontal_avg, vertical_avg)
    short_mm = min(float(a4_width_mm), float(a4_height_mm))
    long_mm = max(float(a4_width_mm), float(a4_height_mm))

    short_edge_mm_per_px = short_mm / max(1e-6, short_px)
    long_edge_mm_per_px = long_mm / max(1e-6, long_px)

    contour_area_px = float(quad_info.get("cleaned_contour_area_px") or quad_info.get("contour_area_px") or abs(cv2.contourArea(quad)))
    paper_area_px = float(abs(cv2.contourArea(quad)))
    mask_area_px = int((_binary_mask(paper_mask) > 0).sum())
    a4_area_mm2 = short_mm * long_mm
    area_mm_per_px = float((a4_area_mm2 / max(1.0, paper_area_px)) ** 0.5)
    ...
    return {
        ...
        "paper_quad_area_px": float(paper_area_px),
        ...
        "scale_method": method,
        "paper_rect_mode": str(paper_rect_mode),
        "paper_quad_method": str(quad_info.get("method")),
        "force_a4_aspect_ratio": bool(force_a4_aspect_ratio),
        "paper_quad_points_px": [[float(x), float(y)] for x, y in quad],
        "paper_rect_quality": quad_info.get("quality"),
        "paper_min_area_rect": {
            "box_points": [[float(x), float(y)] for x, y in quad],
        },
    }
...
def get_quad_metrics(quad: np.ndarray, a4_width_mm: float, a4_height_mm: float) -> Dict[str, Any]:
    """计算 A4 四边形的边长、梯形偏差和长宽比例偏差。"""
    q = np.asarray(quad, dtype=np.float32).reshape(4, 2)
    top = _distance(q[0], q[1])
    right = _distance(q[1], q[2])
    bottom = _distance(q[2], q[3])
    left = _distance(q[3], q[0])

    horizontal_avg = max(1e-6, (top + bottom) / 2.0)
    vertical_avg = max(1e-6, (left + right) / 2.0)

    trapezoid_delta = max(
        abs(top - bottom) / horizontal_avg,
        abs(left - right) / vertical_avg,
    )

    observed_long = max(horizontal_avg, vertical_avg)
    observed_short = max(1e-6, min(horizontal_avg, vertical_avg))
    observed_ratio = observed_long / observed_short
    a4_ratio = max(float(a4_width_mm), float(a4_height_mm)) / max(1e-6, min(float(a4_width_mm), float(a4_height_mm)))
    aspect_error = abs(observed_ratio - a4_ratio) / max(1e-6, a4_ratio)

    # 判断当前四边形横向边对应 A4 长边还是短边。
    # 如果横向平均边更长，则目标矩形宽度使用 A4 长边，否则使用 A4 短边。
    if horizontal_avg >= vertical_avg:
        dst_w_mm = max(float(a4_width_mm), float(a4_height_mm))
        dst_h_mm = min(float(a4_width_mm), float(a4_height_mm))
    else:
        dst_w_mm = min(float(a4_width_mm), float(a4_height_mm))
        dst_h_mm = max(float(a4_width_mm), float(a4_height_mm))

    px_per_mm_candidates = [
        top / max(1e-6, dst_w_mm),
        bottom / max(1e-6, dst_w_mm),
        left / max(1e-6, dst_h_mm),
        right / max(1e-6, dst_h_mm),
    ]
    px_per_mm_auto = float(np.mean(px_per_mm_candidates))

    return {
        "top_px": float(top),
        "right_px": float(right),
        "bottom_px": float(bottom),
        "left_px": float(left),
        "horizontal_avg_px": float(horizontal_avg),
        "vertical_avg_px": float(vertical_avg),
        "observed_aspect_ratio": float(observed_ratio),
        "a4_aspect_ratio": float(a4_ratio),
        "aspect_error": float(aspect_error),
        "trapezoid_delta": float(trapezoid_delta),
        "dst_w_mm": float(dst_w_mm),
        "dst_h_mm": float(dst_h_mm),
        "px_per_mm_auto": float(px_per_mm_auto),
    }


def find_paper_quad_from_mask(
    paper_mask: np.ndarray,
    a4_width_mm: float = 210.0,
    a4_height_mm: float = 297.0,
    paper_rect_mode: str = "robust_fit",
    ...
    fallback_when_deformed: str = "robust_fit",
) -> Dict[str, Any]:
    """
    从 paper mask 中提取用于 A4 透视校正的四边形。

    v5 的核心变化：不直接相信原始 paper mask 的毛边。
    - 先清理 paper mask。
    - 再根据模式选择原始四边形、最小外接矩形或强制 A4 比例矩形。
    - robust_fit 默认使用强制 A4 比例矩形，适合处理轻微翘角、边缘毛刺、局部分割不准。
    """
    raw_contour = _largest_contour_from_mask(paper_mask)
    if raw_contour is None or cv2.contourArea(raw_contour) <= 1:
        raise RuntimeError("无法做透视校正：paper mask 为空，无法提取 A4 四角")

    cleaned_mask = clean_paper_mask_for_rect(
        paper_mask=paper_mask,
        close_kernel_size=int(close_kernel_size),
        open_kernel_size=int(open_kernel_size),
    )
    contour = _largest_contour_from_mask(cleaned_mask)
    if contour is None or cv2.contourArea(contour) <= 1:
        contour = raw_contour
        cleaned_mask = (_binary_mask(paper_mask) * 255).astype(np.uint8)

    contour_area = float(abs(cv2.contourArea(contour)))
    raw_contour_area = float(abs(cv2.contourArea(raw_contour)))
    if contour_area <= 1:
        raise RuntimeError("无法做透视校正：paper 轮廓面积异常")

    raw_quad_info = _legacy_find_paper_quad_from_contour(raw_contour)
    cleaned_quad_info = _legacy_find_paper_quad_from_contour(contour)
    forced_quad_info = build_forced_a4_quad_from_contour(
        contour=contour,
        a4_width_mm=float(a4_width_mm),
        a4_height_mm=float(a4_height_mm),
    )
    ...
    min_rect_quad = order_quad_points(cv2.boxPoints(min_rect).astype(np.float32))
    min_rect_info = {
        "quad": min_rect_quad,
        "method": "min_area_rect_cleaned",
        "contour_area_px": float(contour_area),
    }

    mode = (paper_rect_mode or "robust_fit").strip().lower()
    if mode in {"raw", "legacy", "v4"}:
        chosen = raw_quad_info
    elif mode in {"approx", "approx_poly", "poly"}:
        chosen = cleaned_quad_info
    elif mode in {"min_area_rect", "rect"}:
        chosen = forced_quad_info if force_a4_aspect_ratio else min_rect_info
    elif mode in {"robust", "robust_fit", "force_a4", "a4"}:
        # 默认使用标准 A4 比例矩形。该模式对 paper mask 轻微变形最稳。
        chosen = forced_quad_info if force_a4_aspect_ratio else cleaned_quad_info
    else:
        raise RuntimeError("paper_rect_mode 只支持 robust_fit / approx_poly / min_area_rect / raw")

    quad = order_quad_points(np.asarray(chosen["quad"], dtype=np.float32).reshape(4, 2))

    forced_area = float(abs(cv2.contourArea(np.asarray(forced_quad_info["quad"], dtype=np.float32))))
    cleaned_fill_ratio = float(contour_area / max(1.0, forced_area))
    raw_to_clean_area_ratio = float(raw_contour_area / max(1.0, contour_area))
    quality = max(0.0, min(1.0, cleaned_fill_ratio))

    # 对严重异常的 paper mask 做保护，避免把钢板拉坏。
    fallback = (fallback_when_deformed or "robust_fit").strip().lower()
    if quality_threshold and quality < float(quality_threshold):
        if fallback in {"no_perspective", "disable", "off"}:
            return {
                "quad": quad,
                "method": str(chosen.get("method")),
                "disabled_by_quality": True,
                "quality": float(quality),
                "quality_threshold": float(quality_threshold),
                "message": "paper 矩形拟合质量低，按配置不做透视校正",
                "cleaned_mask": cleaned_mask,
                "source_quad_mask": make_quad_mask(cleaned_mask.shape[:2], quad),
                "auto_decision_quad": np.asarray(raw_quad_info["quad"], dtype=np.float32),
                "cleaned_contour_area_px": float(contour_area),
                "raw_contour_area_px": float(raw_contour_area),
            }
        if fallback in {"raw", "legacy"}:
            chosen = raw_quad_info
            quad = order_quad_points(np.asarray(chosen["quad"], dtype=np.float32).reshape(4, 2))
    ...
    result = {
        "quad": quad,
        "method": str(chosen.get("method")),
        "paper_rect_mode": mode,
        "force_a4_aspect_ratio": bool(force_a4_aspect_ratio),
        "quality": float(quality),
        "quality_threshold": float(quality_threshold),
        "cleaned_contour_area_px": float(contour_area),
        "raw_contour_area_px": float(raw_contour_area),
        "cleaned_fill_ratio_against_forced_rect": float(cleaned_fill_ratio),
        "raw_to_clean_area_ratio": float(raw_to_clean_area_ratio),
        "epsilon_factor": chosen.get("epsilon_factor"),
        "area_error_ratio": chosen.get("area_error_ratio"),
        "contour_area_px": float(contour_area),
        "cleaned_mask": cleaned_mask,
        "source_quad_mask": make_quad_mask(cleaned_mask.shape[:2], quad),
        "auto_decision_quad": auto_decision_quad,
        "raw_quad_method": str(raw_quad_info.get("method")),
        "raw_quad_points_px": [[float(x), float(y)] for x, y in np.asarray(raw_quad_info["quad"], dtype=np.float32)],
        "forced_quad_points_px": [[float(x), float(y)] for x, y in np.asarray(forced_quad_info["quad"], dtype=np.float32)],
        "min_rect_quad_points_px": [[float(x), float(y)] for x, y in min_rect_quad],
    }

    for key, value in forced_quad_info.items():
        if key not in {"quad"}:
            result[f"forced_{key}"] = value

    return result


def _build_perspective_matrix_and_size(
    ...
    """
    构建透视矩阵，并自动扩大输出画布，避免钢板被裁切。

    先把 A4 映射到一个真实比例矩形，再计算整张原图四角在新坐标系下的位置，
    最后加平移矩阵把所有内容移到正坐标区域。
    """
    h, w = image_shape[:2]
    dst_w_px = max(10, int(round(float(dst_w_mm) * float(px_per_mm))))
    dst_h_px = max(10, int(round(float(dst_h_mm) * float(px_per_mm))))

    dst_quad = np.array([
        [0.0, 0.0],
        [float(dst_w_px - 1), 0.0],
        [float(dst_w_px - 1), float(dst_h_px - 1)],
        [0.0, float(dst_h_px - 1)],
    ], dtype=np.float32)

    h_mat = cv2.getPerspectiveTransform(np.asarray(source_quad, dtype=np.float32), dst_quad)
    ...
    warped_corners = cv2.perspectiveTransform(image_corners, h_mat).reshape(-1, 2)

    all_x = list(warped_corners[:, 0]) + list(dst_quad[:, 0])
    all_y = list(warped_corners[:, 1]) + list(dst_quad[:, 1])
    ...
    output_w = max(10, int(np.ceil(max_x - min_x + margin * 2)))
    output_h = max(10, int(np.ceil(max_y - min_y + margin * 2)))
    ...
    final_h_mat = translate_mat @ h_mat

    return {
        "homography": final_h_mat,
        "output_size": [int(output_w), int(output_h)],
        "dst_paper_size_px": [int(dst_w_px), int(dst_h_px)],
        "dst_paper_size_mm": [float(dst_w_mm), float(dst_h_mm)],
        "translation_px": [float(translate_x), float(translate_y)],
        "projected_image_bbox_before_translation": [float(min_x), float(min_y), float(max_x), float(max_y)],
    }


def _build_homography_plan_from_point_pairs(
    image_shape: Tuple[int, int],
    src_points_px: np.ndarray,
    dst_points_plane_px: np.ndarray,
    dst_board_w_mm: float,
    dst_board_h_mm: float,
    px_per_mm: float,
    margin_px: int,
) -> Dict[str, Any]:
    """
    根据多个图像点到真实平面点的对应关系构建透视方案。

    ArUco 标定板会提供多个 marker 角点，这里用 findHomography 得到图像到毫米平面的映射，
    再扩大输出画布，避免 A4 外面的钢板区域被裁切。
    """
    h, w = image_shape[:2]
    src = np.asarray(src_points_px, dtype=np.float32).reshape(-1, 2)
    dst = np.asarray(dst_points_plane_px, dtype=np.float32).reshape(-1, 2)
    if len(src) < 4 or len(dst) < 4:
        raise RuntimeError("构建 ArUco 透视矩阵至少需要 4 组角点")

    if len(src) >= 8:
        h_mat, inlier_mask = cv2.findHomography(src, dst, cv2.RANSAC, 3.0)
    else:
        h_mat, inlier_mask = cv2.findHomography(src, dst, 0)
    if h_mat is None:
        raise RuntimeError("ArUco 角点无法计算 homography")

    dst_w_px = max(10, int(round(float(dst_board_w_mm) * float(px_per_mm))))
    dst_h_px = max(10, int(round(float(dst_board_h_mm) * float(px_per_mm))))
    dst_board_quad = np.array([
        [0.0, 0.0],
        [float(dst_w_px - 1), 0.0],
        [float(dst_w_px - 1), float(dst_h_px - 1)],
        [0.0, float(dst_h_px - 1)],
    ...
    warped_corners = cv2.perspectiveTransform(image_corners, h_mat.astype(np.float64)).reshape(-1, 2)

    all_x = list(warped_corners[:, 0]) + list(dst_board_quad[:, 0])
    all_y = list(warped_corners[:, 1]) + list(dst_board_quad[:, 1])
    min_x = float(min(all_x))
    max_x = float(max(all_x))
    min_y = float(min(all_y))
    max_y = float(max(all_y))
    ...
    final_h_mat = translate_mat @ h_mat.astype(np.float64)

    # 反投影 A4 标定板四个外角到原图，用于保存调试 mask。
    source_board_quad = None
    try:
        inv_h = np.linalg.inv(h_mat.astype(np.float64))
        src_board = cv2.perspectiveTransform(dst_board_quad.reshape(-1, 1, 2), inv_h).reshape(-1, 2)
        source_board_quad = order_quad_points(src_board.astype(np.float32))
    except Exception:
        source_board_quad = None
    ...
    return {
        ...
        "output_size": [int(output_w), int(output_h)],
        "dst_paper_size_px": [int(dst_w_px), int(dst_h_px)],
        "dst_paper_size_mm": [float(dst_board_w_mm), float(dst_board_h_mm)],
        "translation_px": [float(translate_x), float(translate_y)],
        "projected_image_bbox_before_translation": [float(min_x), float(min_y), float(max_x), float(max_y)],
        "source_board_quad_points_px": None if source_board_quad is None else [[float(x), float(y)] for x, y in source_board_quad],
        "homography_inlier_count": inliers,
        "homography_point_count": int(len(src)),
    }


def build_aruco_perspective_correction_plan(
    image_rgb: np.ndarray,
    req: ProcessRequest,
) -> Dict[str, Any]:
    """
    基于 A4 四角 ArUco 标定板生成透视校正方案。

    这个方案不依赖白纸边缘，也不依赖 SAM2 的 paper mask，适合 A4 纸边缘翘起、阴影、毛边导致普通 A4 拟合不稳定的情况。
    """
    if not bool(req.enable_perspective_correction):
        return {"enabled": False, "reason": "perspective_disabled"}
    if not bool(req.enable_aruco_calibration):
        return {"enabled": False, "reason": "aruco_disabled"}

    mode = (req.perspective_mode or "auto").strip().lower()
    if mode in {"never", "none", "off", "false"}:
        return {"enabled": False, "reason": "perspective_disabled_by_mode"}

    detected_result = detect_aruco_markers(image_rgb, req.aruco_dictionary_name)
    detected = detected_result["detected"]
    wanted_ids = [int(x) for x in req.aruco_marker_ids[:4]]
    used = [m for m in detected if int(m["id"]) in set(wanted_ids)]
    used_ids = sorted({int(m["id"]) for m in used})

    min_markers = max(1, int(req.aruco_min_markers))
    if len(used_ids) < min_markers:
        return {
            "enabled": False,
            "reason": "not_enough_aruco_markers",
            "detected_ids": detected_result.get("detected_ids", []),
            "used_ids": used_ids,
            "required_marker_ids": wanted_ids,
            "aruco_min_markers": int(min_markers),
        }

    src_points = []
    dst_points = []
    used_markers = []
    px_per_mm = float(req.perspective_px_per_mm) if req.perspective_px_per_mm is not None else 5.0
    px_per_mm = max(0.1, px_per_mm)

    for marker in used:
        marker_id = int(marker["id"])
        obj_mm = get_aruco_marker_object_corners_mm(
            marker_id=marker_id,
            marker_ids=wanted_ids,
            board_width_mm=float(req.aruco_board_width_mm),
            board_height_mm=float(req.aruco_board_height_mm),
            marker_size_mm=float(req.aruco_marker_size_mm),
            marker_margin_mm=float(req.aruco_marker_margin_mm),
        )
        if obj_mm is None:
            continue
        img_corners = np.asarray(marker["corners_px"], dtype=np.float32).reshape(4, 2)
        dst_corners = obj_mm * px_per_mm
        src_points.extend(img_corners.tolist())
        dst_points.extend(dst_corners.tolist())
        used_markers.append({
            "id": marker_id,
            "image_corners_px": [[float(x), float(y)] for x, y in img_corners],
            "object_corners_mm": [[float(x), float(y)] for x, y in obj_mm],
        })

    if len(src_points) < 4:
        return {"enabled": False, "reason": "not_enough_aruco_corner_pairs", "used_ids": used_ids}

    plan_core = _build_homography_plan_from_point_pairs(
        image_shape=image_rgb.shape[:2],
        src_points_px=np.asarray(src_points, dtype=np.float32),
        dst_points_plane_px=np.asarray(dst_points, dtype=np.float32),
        dst_board_w_mm=float(req.aruco_board_width_mm),
        dst_board_h_mm=float(req.aruco_board_height_mm),
        px_per_mm=px_per_mm,
        margin_px=int(req.perspective_margin_px),
    )

    max_pixels = max(1000000, int(req.perspective_max_output_pixels))
    output_w, output_h = plan_core["output_size"]
    output_pixels = int(output_w * output_h)
    scaled_by_limit = False

    if output_pixels > max_pixels:
        scale = float((max_pixels / max(1, output_pixels)) ** 0.5)
        px_per_mm = max(0.1, px_per_mm * scale)
        scaled_by_limit = True
        src_points = []
        dst_points = []
        for marker in used:
            marker_id = int(marker["id"])
            ...
            if obj_mm is None:
                continue
            src_points.extend(np.asarray(marker["corners_px"], dtype=np.float32).reshape(4, 2).tolist())
            dst_points.extend((obj_mm * px_per_mm).tolist())
        plan_core = _build_homography_plan_from_point_pairs(
            image_shape=image_rgb.shape[:2],
            src_points_px=np.asarray(src_points, dtype=np.float32),
            dst_points_plane_px=np.asarray(dst_points, dtype=np.float32),
    ...
    board_quad = plan_core.get("source_board_quad_points_px")
    metrics = {}
    if board_quad:
        metrics = get_quad_metrics(np.asarray(board_quad, dtype=np.float32), float(req.aruco_board_width_mm), float(req.aruco_board_height_mm))

    plan_core.update({
        "enabled": True,
        "mode": mode,
        "reason": "aruco_board_detected",
        "calibration_method": "aruco",
        "quad_method": "aruco_marker_corners",
        "dictionary_name": req.aruco_dictionary_name,
        "required_marker_ids": wanted_ids,
        "detected_ids": detected_result.get("detected_ids", []),
        "used_ids": used_ids,
        "used_marker_count": int(len(used_ids)),
        "used_markers": used_markers,
        "source_quad_points_px": board_quad,
        "source_board_quad_points_px": board_quad,
        "metrics": metrics,
        "px_per_mm": float(px_per_mm),
        "mm_per_px": float(1.0 / px_per_mm),
        "output_pixels": int(output_pixels),
        "scaled_by_max_output_pixels": bool(scaled_by_limit),
        "max_output_pixels": int(max_pixels),
    })
    return plan_core


def build_perspective_correction_plan(
    paper_mask: np.ndarray,
    image_shape: Tuple[int, int],
    req: ProcessRequest,
) -> Dict[str, Any]:
    """根据 A4纸 mask 生成透视校正方案。"""
    mode = (req.perspective_mode or "auto").strip().lower()
    if not bool(req.enable_perspective_correction) or mode in {"never", "none", "off", "false"}:
        return {
            "enabled": False,
            "mode": mode,
    ...
    if paper_mask is None or int((paper_mask > 0).sum()) <= 0:
        raise RuntimeError("无法做透视校正：没有可用 paper mask")

    quad_info = find_paper_quad_from_mask(
        paper_mask=paper_mask,
        a4_width_mm=float(req.a4_width_mm),
        a4_height_mm=float(req.a4_height_mm),
        paper_rect_mode=req.paper_rect_mode,
        force_a4_aspect_ratio=bool(req.force_a4_aspect_ratio),
        close_kernel_size=int(req.paper_rect_clean_kernel),
        open_kernel_size=int(req.paper_rect_open_kernel),
        quality_threshold=float(req.paper_rect_quality_threshold),
        fallback_when_deformed=req.fallback_when_paper_deformed,
    )
    quad = quad_info["quad"]

    if quad_info.get("disabled_by_quality"):
        return {
            "enabled": False,
            "mode": mode,
            "reason": "paper_rect_quality_too_low",
            "quad_method": quad_info.get("method"),
            "paper_rect_mode": req.paper_rect_mode,
            "quality": quad_info.get("quality"),
            "quality_threshold": quad_info.get("quality_threshold"),
            "source_quad_points_px": [[float(x), float(y)] for x, y in np.asarray(quad, dtype=np.float32)],
        }

    metrics = get_quad_metrics(quad, float(req.a4_width_mm), float(req.a4_height_mm))
    decision_quad = np.asarray(quad_info.get("auto_decision_quad", quad), dtype=np.float32)
    decision_metrics = get_quad_metrics(decision_quad, float(req.a4_width_mm), float(req.a4_height_mm))

    trapezoid_delta = float(decision_metrics["trapezoid_delta"])
    aspect_error = float(decision_metrics["aspect_error"])
    need_by_auto = (
        trapezoid_delta >= float(req.perspective_auto_trapezoid_threshold)
        or aspect_error >= float(req.perspective_auto_aspect_threshold)
        or (req.paper_rect_mode or "").strip().lower() in {"robust", "robust_fit", "force_a4", "a4"}
    )

    if mode == "auto" and not need_by_auto:
        return {
            "enabled": False,
            "mode": mode,
            "reason": "paper_shape_close_to_rectangular",
            "quad_method": quad_info["method"],
            "paper_rect_mode": req.paper_rect_mode,
            "source_quad_points_px": [[float(x), float(y)] for x, y in quad],
            "auto_decision_quad_points_px": [[float(x), float(y)] for x, y in decision_quad],
            "metrics": metrics,
            "auto_decision_metrics": decision_metrics,
        }

    if mode not in {"auto", "always", "force", "on", "true"}:
        raise RuntimeError("perspective_mode 只支持 auto / always / never")

    px_per_mm = float(req.perspective_px_per_mm) if req.perspective_px_per_mm is not None else float(metrics["px_per_mm_auto"])
    px_per_mm = max(0.1, px_per_mm)

    plan_core = _build_perspective_matrix_and_size(
        image_shape=image_shape,
        source_quad=quad,
        dst_w_mm=float(metrics["dst_w_mm"]),
        dst_h_mm=float(metrics["dst_h_mm"]),
        px_per_mm=px_per_mm,
        margin_px=int(req.perspective_margin_px),
    )
    ...
    plan_core.update({
        "enabled": True,
        "mode": mode,
        "reason": "forced" if mode in {"always", "force", "on", "true"} else "auto_detected_perspective",
        "quad_method": quad_info["method"],
        "paper_rect_mode": req.paper_rect_mode,
        "force_a4_aspect_ratio": bool(req.force_a4_aspect_ratio),
        "paper_rect_quality": quad_info.get("quality"),
...
def warp_mask_by_perspective(mask: np.ndarray, plan: Dict[str, Any]) -> np.ndarray:
    """按透视方案转换 mask。"""
    if not plan.get("enabled"):
        raise RuntimeError("透视方案未启用，不能 warp mask")
    output_w, output_h = [int(x) for x in plan["output_size"]]
    warped = cv2.warpPerspective(
        (_binary_mask(mask) * 255).astype(np.uint8),
        np.asarray(plan["homography"], dtype=np.float64),
        (output_w, output_h),
...
def warp_image_by_perspective(image_rgb: np.ndarray, plan: Dict[str, Any]) -> np.ndarray:
    """按透视方案转换原图，用于调试预览。"""
    if not plan.get("enabled"):
        raise RuntimeError("透视方案未启用，不能 warp image")
    output_w, output_h = [int(x) for x in plan["output_size"]]
...
def build_a4_scale_from_perspective_plan(plan: Dict[str, Any], req: ProcessRequest) -> Dict[str, Any]:
    """根据透视校正输出比例构建 A4 标定信息。"""
    dst_w_px, dst_h_px = [int(x) for x in plan["dst_paper_size_px"]]
    dst_w_mm, dst_h_mm = [float(x) for x in plan["dst_paper_size_mm"]]
    px_per_mm = float(plan["px_per_mm"])
    mm_per_px = float(plan["mm_per_px"])
    short_px = float(min(dst_w_px, dst_h_px))
    long_px = float(max(dst_w_px, dst_h_px))
    short_mm = float(min(dst_w_mm, dst_h_mm))
    long_mm = float(max(dst_w_mm, dst_h_mm))

    return {
        "a4_width_mm": float(dst_w_mm),
        "a4_height_mm": float(dst_h_mm),
        "a4_short_mm": short_mm,
        "a4_long_mm": long_mm,
        "paper_short_px": short_px,
        "paper_long_px": long_px,
        "short_edge_mm_per_px": float(short_mm / max(1e-6, short_px)),
        "long_edge_mm_per_px": float(long_mm / max(1e-6, long_px)),
        "area_mm_per_px": float(mm_per_px),
        "mm_per_px": float(mm_per_px),
        "px_per_mm": float(px_per_mm),
        "scale_method": "perspective_a4_homography",
        "paper_rectified_size_px": [int(dst_w_px), int(dst_h_px)],
        "paper_rectified_size_mm": [float(dst_w_mm), float(dst_h_mm)],
    }


def generate_dxf_and_dimensions_with_fixed_scale(
    plate_mask: np.ndarray,
    output_dxf_path: str | Path,
    req: ProcessRequest,
    mm_per_px: float,
    a4_scale: Dict[str, Any],
    perspective_info: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """使用已知 mm_per_px 生成 DXF 和尺寸信息。"""
    ...
    dxf_geometry = convert_contours_to_mm_polylines(contour_data, mm_per_px=float(mm_per_px))
    ...
    plate_rect_dimension = calculate_plate_rotated_rect_dimension(plate_mask, mm_per_px=float(mm_per_px))

    return {
        "dxf_path": str(dxf_path),
        "dimensions": {
            "perspective": perspective_info or {"enabled": False},
            "a4": a4_scale,
            "plate": {
...
def strip_perspective_plan_for_json(plan: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """移除 homography 等大对象，保留接口和日志需要的诊断信息。"""
    if not plan:
        return {"enabled": False}
    out = {k: v for k, v in plan.items() if k != "homography"}
...
def generate_dxf_and_dimensions(
    ...
    a4_scale = calculate_a4_scale_from_paper_mask(
        ...
        paper_rect_mode=req.paper_rect_mode,
        force_a4_aspect_ratio=bool(req.force_a4_aspect_ratio),
        paper_rect_clean_kernel=int(req.paper_rect_clean_kernel),
        paper_rect_open_kernel=int(req.paper_rect_open_kernel),
    )
...
def process_image_rgb(
    ...
    camera_undistort_info: Dict[str, Any] = {"enabled": False}
    camera_undistorted_path = None
    if bool(req.enable_camera_undistort):
        if not req.camera_calibration_path:
            raise RuntimeError("enable_camera_undistort=true 时必须传 camera_calibration_path")
        image_rgb, camera_undistort_info = undistort_image_rgb(
            image_rgb=image_rgb,
            calibration_path=req.camera_calibration_path,
            alpha=float(req.camera_undistort_alpha),
        )
        camera_undistorted_path = save_image_rgb(image_rgb, run_dir / "input_undistorted_before_yolo_sam2.png")
    ...
    outputs: Dict[str, Any] = {
        ...
        "same_image_rgb": True,
        "camera_undistort": camera_undistort_info,
        "camera_undistorted_path": camera_undistorted_path,
        "targets": {},
        "paper": None,
    ...
    if camera_undistorted_path:
        mask_paths["camera_undistorted_image"] = camera_undistorted_path
    ...
    if req.enable_dxf and plate_for_dxf is not None:
        try:
            calibration_mode = (req.calibration_mode or "auto").strip().lower()
            if calibration_mode not in {"auto", "aruco", "paper"}:
                raise RuntimeError("calibration_mode 只支持 auto / aruco / paper")

            perspective_plan: Optional[Dict[str, Any]] = None
            aruco_plan_result: Optional[Dict[str, Any]] = None

            # v6：优先使用 ArUco 标定板。普通 A4 纸边缘变形时，ArUco 角点比白纸边缘可靠得多。
            if calibration_mode in {"auto", "aruco"} and bool(req.enable_aruco_calibration):
                try:
                    aruco_plan_result = build_aruco_perspective_correction_plan(
                        image_rgb=image_rgb,
                        req=req,
                    )
                    if aruco_plan_result.get("enabled"):
                        perspective_plan = aruco_plan_result
                except Exception as aruco_error:
                    aruco_plan_result = {
                        "enabled": False,
                        "reason": "aruco_exception",
                        "message": str(aruco_error),
                    }
                    if calibration_mode == "aruco":
                        raise

            if perspective_plan is None and calibration_mode == "aruco":
                raise RuntimeError(f"ArUco 标定失败，且 calibration_mode=aruco，不回退普通 A4。detail={aruco_plan_result}")

            # ArUco 失败或用户指定 paper 时，回退到 v5 的普通 A4 paper mask 方案。
            if perspective_plan is None:
                if paper_mask is None or int((paper_mask > 0).sum()) <= 0:
                    raise RuntimeError(f"无法计算尺寸和 DXF：没有可用 paper mask，且 ArUco 未启用或未检测成功。aruco={aruco_plan_result}")

                perspective_plan = build_perspective_correction_plan(
                    paper_mask=paper_mask,
                    image_shape=image_rgb.shape[:2],
                    req=req,
                )
                perspective_plan["calibration_method"] = "paper"
                if aruco_plan_result is not None:
                    perspective_plan["aruco_fallback"] = strip_perspective_plan_for_json(aruco_plan_result)

                # v5：保存清理后的 paper mask 和稳健 A4 拟合矩形，方便现场判断 A4 是否校正准确。
                if req.save_paper_rect_masks:
                    paper_cleaned_mask = clean_paper_mask_for_rect(
                        paper_mask=paper_mask,
                        close_kernel_size=int(req.paper_rect_clean_kernel),
                        open_kernel_size=int(req.paper_rect_open_kernel),
                    )
                    paper_cleaned_path = save_mask(
                        paper_cleaned_mask,
                        run_dir / "perspective" / f"{stem}_paper_cleaned_mask.png",
                    )
                    mask_paths["paper_cleaned_mask"] = paper_cleaned_path

                    source_quad_points = perspective_plan.get("source_quad_points_px")
                    if source_quad_points:
                        paper_fitted_rect_mask = make_quad_mask(
                            image_rgb.shape[:2],
                            np.asarray(source_quad_points, dtype=np.float32),
                        )
                        paper_fitted_rect_path = save_mask(
                            paper_fitted_rect_mask,
                            run_dir / "perspective" / f"{stem}_paper_fitted_a4_rect_mask.png",
                        )
                        mask_paths["paper_fitted_a4_rect_mask"] = paper_fitted_rect_path
            else:
                # v6：保存 ArUco 检测调试图和反投影出来的 A4 标定板区域。
                if bool(req.save_aruco_debug_images):
                    used_markers_for_debug = []
                    for marker in perspective_plan.get("used_markers", []):
                        used_markers_for_debug.append({
                            "id": int(marker["id"]),
                            "corners_px": np.asarray(marker["image_corners_px"], dtype=np.float32),
                        })
                    if used_markers_for_debug:
                        aruco_debug_path = save_aruco_debug_image(
                            image_rgb=image_rgb,
                            detected_markers=used_markers_for_debug,
                            output_path=run_dir / "aruco" / f"{stem}_aruco_debug.jpg",
                        )
                        mask_paths["aruco_debug_image"] = aruco_debug_path

                source_board_quad = perspective_plan.get("source_board_quad_points_px") or perspective_plan.get("source_quad_points_px")
                if source_board_quad:
                    aruco_board_quad_mask = make_quad_mask(
                        image_rgb.shape[:2],
                        np.asarray(source_board_quad, dtype=np.float32),
                    )
                    aruco_board_quad_path = save_mask(
                        aruco_board_quad_mask,
                        run_dir / "aruco" / f"{stem}_aruco_board_quad_mask.png",
                    )
                    mask_paths["aruco_board_quad_mask"] = aruco_board_quad_path

            if perspective_plan.get("enabled"):
                # 斜拍时使用标定点做透视校正，然后在校正后的 mask 上计算尺寸和 DXF。
                plate_perspective_mask = warp_mask_by_perspective(plate_for_dxf, perspective_plan)

                plate_perspective_path = save_mask(
                    plate_perspective_mask,
                    run_dir / "perspective" / f"{stem}_plate_perspective_mask.png",
                )
                mask_paths["plate_perspective_mask"] = plate_perspective_path

                if paper_mask is not None and int((paper_mask > 0).sum()) > 0:
                    paper_perspective_mask = warp_mask_by_perspective(paper_mask, perspective_plan)
                    paper_perspective_path = save_mask(
                        paper_perspective_mask,
                        run_dir / "perspective" / f"{stem}_paper_perspective_mask.png",
                    )
                    mask_paths["paper_perspective_mask"] = paper_perspective_path

                if req.save_debug_images:
                    perspective_image = warp_image_by_perspective(image_rgb, perspective_plan)
                    perspective_image_path = save_image_rgb(
                        perspective_image,
                        run_dir / "perspective" / f"{stem}_perspective_preview.png",
                    )
                    mask_paths["perspective_preview_image"] = perspective_image_path

                perspective_info = strip_perspective_plan_for_json(perspective_plan)
                a4_scale = build_a4_scale_from_perspective_plan(perspective_plan, req)
                dxf_result = generate_dxf_and_dimensions_with_fixed_scale(
                    plate_mask=plate_perspective_mask,
                    output_dxf_path=run_dir / "dxf" / f"{stem}_plate_perspective.dxf",
                    req=req,
                    mm_per_px=float(perspective_plan["mm_per_px"]),
                    a4_scale=a4_scale,
                    perspective_info=perspective_info,
                )
            else:
                # 轻微倾斜或关闭透视校正时，使用普通 A4 比例算法。
                if paper_mask is None or int((paper_mask > 0).sum()) <= 0:
                    raise RuntimeError("透视校正未启用且没有 paper mask，无法计算普通 A4 比例")
                ...
                dxf_result["dimensions"]["perspective"] = strip_perspective_plan_for_json(perspective_plan)
...
# FastAPI
# =========================
app = FastAPI(title="YOLO + SAM2 Plate/Paper Service", version="1.4.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def health():
    return {
        ...
        "aruco_available": bool(getattr(cv2, "aruco", None) is not None),
    }


@app.post("/generate-aruco-a4-board")
def generate_aruco_a4_board(req: ArucoBoardRequest):
    """生成 A4 四角 ArUco 标定板 PNG。打印时必须选择 100% 原尺寸，不要自适应页面。"""
    try:
        output_root = Path(req.output_root or RUNTIME_CONFIG.get("output_root") or DEFAULT_OUTPUT_ROOT)
        out_dir = output_root / "aruco_boards"
        out_dir.mkdir(parents=True, exist_ok=True)

        board = generate_a4_aruco_board_image(
            dictionary_name=req.dictionary_name,
            marker_ids=req.marker_ids,
            board_width_mm=float(req.board_width_mm),
            board_height_mm=float(req.board_height_mm),
            marker_size_mm=float(req.marker_size_mm),
            marker_margin_mm=float(req.marker_margin_mm),
            dpi=int(req.dpi),
        )

        filename = Path(req.filename).name or "aruco_a4_board.png"
        if not filename.lower().endswith(".png"):
            filename += ".png"
        image_path = save_image_rgb(board, out_dir / filename)

        layout = {
            "dictionary_name": req.dictionary_name,
            "marker_ids": [int(x) for x in req.marker_ids[:4]],
            "marker_id_order": "left_top, right_top, right_bottom, left_bottom",
            "board_width_mm": float(req.board_width_mm),
            "board_height_mm": float(req.board_height_mm),
            "marker_size_mm": float(req.marker_size_mm),
            "marker_margin_mm": float(req.marker_margin_mm),
            "dpi": int(req.dpi),
            "print_note": "打印时选择 100% 原尺寸，不要选择适应页面、缩放页面或填满页面。建议塑封或贴到硬板上保持平整。",
        }
        layout_path = out_dir / (Path(filename).stem + "_layout.json")
